...
function go() {
  ...
  let number = { num };
  let arr = JSON.parse(localStorage.getItem("medibuddyuser"));
  if (arr == null) {
    arr = [];
  } else {
    arr = JSON.parse(localStorage.getItem("medibuddyuser"));
  }

  arr.push(number);
  localStorage.setItem("medibuddyuser", JSON.stringify(arr));
